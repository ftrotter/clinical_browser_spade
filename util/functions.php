...
//returns false if $IN does not have a proper api_key
function api_key_check($IN){
	//dont do this if the global is not set...
	if(isset($GLOBALS['force_api_keys'])){

		$apikey = false;
		if(isset($SESSION['apikey'])){
		...
		if(!$apikey){ //then it is not even there..
			return(false);
		}
		...
		$sql = 
"SELECT * 
FROM  `npi_user`.`api_users` 
WHERE  `key` LIKE  '$apikey'";

		$result = mysql_query($sql) or die("Could not load api key with $sql".mysql_error());

		if(mysql_num_rows($result) > 0){ //we found it...
			return(true);
		}else{
...
function base_url(){
	...
	if(isset($_SERVER['HTTP_HOST'])){ //test for command line...
